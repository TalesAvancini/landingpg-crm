# Plano Definitivo: LEARNINGS — A Memória do H.O.K Forge v2.6

**Versão do documento:** FINAL
**Data:** 2026-04-30
**Origem:** Convergência de 4 fontes (MiMo, Qwen, Flash, Codex) + correções do Humano
...
| `cleanup_specs.py` | **Produtor.** Gera dados sobre specs arquivadas. LEARNINGS consome. |
...
|---|---|---|---|
| **Agregador** (FAIL→SUCCESS) | 100% | Padrões de falha, diffs, recorrências | ~65% |
| **Agregador** (Oracle gaps) | 100% | Queries conf < 0.5, buracos na documentação | ~15% |
| **Humano** (triagem) | Manual | Causas raiz, validação de propostas | ~20% |
...
- **Causa raiz:** [preenchido pelo humano ou "PENDENTE"]
...
| ID | Título | Tipo | Impacto | Status | Última Ocorrência | Contagem |
|---|---|---|---|---|---|---|
| SCAR-... | ... | ... | ... | 🟢/💤/✅/🔍 | ciclo #N | N |
```
...
| **Regras imutáveis** | NUNCA modifica nada em `brain/` exceto `LEARNINGS.md`. |
...
FUNÇÃO PRINCIPAL:
  ...
  3. Parsear HARNESS_LOG.md via safe_parse_log()
     ...
     → Para cada FAIL: spec, check, detalhe, timestamp
  ...
  10. Log de performance
      → "[OK] learnings_aggregator: N scars, N gaps, N propostas. 
         Tempo: X.Xs. Ciclo #N."
```
...
#### Estrutura de Dados de uma Scar
...
#### Mapeamento Fonte → Tipo de Cicatriz
...
| **Execução** | Antes de cada invocação do spec-driver |
| **Timeout** | 2 segundos |
...
O aggregator verifica esse header antes de parsear. Se a versão mudar ou não existir, reporta `[WARN] Schema do HARNESS_LOG.md mudado. Parsing suspenso.`
...
| Aspecto | Especificação |
|---|---|
| **Quando** | Antes de cada invocação do spec-driver |
...
### 4.4. Propostas de Regra

| Aspecto | Especificação |
|---|---|
| **Trigger** | Mesmo padrão >= 3 vezes |
...
| Schema drift nos logs | Header `<!-- HARNESS_LOG_SCHEMA: 1.0 -->`. Aggregator verifica antes de parsear. |
...
9. Aggregator:
   ...
   b. Incrementa ciclo
   c. Parseia HARNESS_LOG.md (safe_parse_log)
   ...
   h. Verifica recorrências → propostas de regra
   i. Aplica decay por ciclos
   j. Escrita atômica (mtime check)
   k. Log de performance
```
...
2. Terminal:
   ...
   [2] Scar #012 — impact_radius: scope_blowout
       ...
3. Humano edita LEARNINGS.md, preenche root_cause
...
| 1.3 | Preencher primeiras 3-5 cicatrizes retroativamente | `brain/LEARNINGS.md` | Médio |
| 1.4 | Formalizar Conventional Commits | `brain/RULES.md` (+§1.4) | Baixo |
| 1.5 | Criar hook commit-msg | `.husky/commit-msg` | Baixo |
| 1.6 | Adicionar schema version ao HARNESS_LOG.md | `HARNESS_LOG.md` (header) | Baixo |
| 1.7 | Adicionar scripts npm | `package.json` (+3 linhas) | Baixo |

**Entregável:** LEARNINGS.md com cicatrizes reais. Conventional Commits ativo. Husky validando. Schema version no log.

**Critério de sucesso:** LEARNINGS.md contém >= 3 scars do histórico real. Commits inválidos bloqueados pelo Husky.
...
| 2.6 | Implementar decay por ciclos | `learnings_aggregator.py` | Médio |
| 2.7 | Implementar detecção de propostas de regra | `learnings_aggregator.py` | Médio |
| 2.8 | Implementar modo --triage (ordenado por urgência) | `learnings_aggregator.py` | Baixo |
| 2.9 | Implementar escrita atômica + mtime + log de performance | `learnings_aggregator.py` | Baixo |
| 2.10 | Testar com dados reais | - | Médio |
| 2.11 | Validar idempotência | - | Baixo |

**Entregável:** Script funcional. `npm run context:learnings` roda sem erros.

**Critério de sucesso:** Gera scars reais. Rodar 2x não duplica. Timeout 5s respeitado. Log de performance presente.
...
| 3.5 | Integrar injeção no fluxo de execução | `run_context.py` / `MASTER_FLOW.md` | Médio |
...
**Critério de sucesso:** >= 50% scars com causa raiz. Propostas avaliadas. Glossários atualizados.
...
| 5.4 | Validar Conventional Commits | - | Baixo |
| 5.5 | Coletar métricas de sucesso | - | Médio |

**Entregável:** LEARNINGS em produção, validado end-to-end.

**Critério de sucesso:** Redução de erros repetidos detectável. Nenhum falso positivo bloqueou commits. Performance dentro do SLO.
...
| 5 | **MODIFICAR** | `brain/RULES.md` | Documento (+§1.4) | 1 |
| 6 | **MODIFICAR** | `.context/_scripts/harness_runner.py" | Script (+25 linhas + schema header) | 3 |
| 7 | **MODIFICAR** | `package.json` | Config (+3 linhas) | 1 |
| 8 | **MODIFICAR** | `brain/SCRIPT_GLOSSARY.md` | Documento | 4 |
| 9 | **MODIFICAR** | `brain/FILE_GLOSSARY.md` | Documento | 4 |

**Total: 4 artefatos novos, 5 modificações.**
...
Decisões tomadas durante o planejamento, com rationale:
...
Fim do plano. Tudo consolidado. Tudo conectado. Nada esquecido.
